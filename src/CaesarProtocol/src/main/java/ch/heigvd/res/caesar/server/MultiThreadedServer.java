        ...
        private class ServantWorker implements Runnable {
            ...
            public void run() {
                ...
                try {
                    ...
                    ////// DEBUT PROTOCOL
                    // 1
                    out.println("Donnez moi la constante du cryptage de cesar (un int)");
                    out.flush();
                    // 4
                    LOG.info("En attente de la constante");
                    String constant = in.readLine();
                    LOG.info("Client asked "+constant+" for constant");
                    offset = Integer.parseInt(constant);
                    // 5
                    out.println("La constante : "+constant+" a été acceptée");
                    out.flush();

                    while ((shouldRun) && (line = in.readLine()) != null) {
                        if (Protocol.decrypt(line, offset).equalsIgnoreCase("bye")) {
                            shouldRun = false;
                        }
                        // @TODO //////////////////////////////

                        LOG.info("Ligne reçue : "+line+" > "+Protocol.decrypt(line, offset));
